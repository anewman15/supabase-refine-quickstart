...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={8}>
    <StepHikeCompact.Details title="Add countries `list` page">
      ...
      The  `countries` data is fetched from the **Supabase** `/countries` endpoint via **refine**'s `useList()` data hook. The `useList()` hook under the hood calls the `list` method of the `dataProvider` object that was generated by `@refinedev/supabase` package.

      More on this in [refine `useList()` docs here](https://refine.dev/docs/api-reference/core/hooks/data/useList/).
  ...
  <StepHikeCompact.Step step={9}>
    <StepHikeCompact.Details title="View countries page">
      ...
      ![refine-countries-list](https://imgbox.com/Uempx4au)
